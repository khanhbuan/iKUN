...
def test_accuracy_v1(model, dataloader, save_img=False):
    ...
    with torch.no_grad():
        ...
        # for batch_idx, data in enumerate(dataloader):
            ...
            inputs = dict(
                img=images.to("cuda"),
                exp=tokenize(expressions).to("cuda")
            )
...
def test_accuracy(model, dataloader, save_img=False):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(tqdm(dataloader)):
            ...
            # forward
            inputs = dict(
                local_img=data['cropped_images'].to("cuda").repeat_interleave(len(expressions), dim=0),
                global_img=data['global_images'].to("cuda").repeat_interleave(len(expressions), dim=0),
                exp=tokenize(expressions).to("cuda"),
            )
...
def test_tracking(model, dataloader):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(tqdm(dataloader)):
            # forward
            inputs = dict(
                local_img=data['cropped_images'].to("cuda"),
                global_img=data['global_images'].to("cuda"),
                exp=tokenize(data['expression_new']).to("cuda"),
            )
