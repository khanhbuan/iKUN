...
for epoch in range(resume_epoch + 1, opt.max_epoch):
    ...
    for batch_idx, data in enumerate(dataloader_train):
        ...
        expression_ids = data['expression_id'].to("cuda")
        # forward
        inputs = dict(
            local_img=data['cropped_images'].to("cuda"),
            global_img=data['global_images'].to("cuda"),
            exp=tokenize(expression).to("cuda"),
        )
        targets = data['target_labels'].view(-1).to("cuda")
    ...
    if (epoch + 1) % opt.eval_frequency == 0:
        ...
        wandb.log({"loss": loss.item(), "precision": '{:.2f}%'.format(p), "recall": '{:.2f}%'.format(r)})
