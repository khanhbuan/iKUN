...
def filter_target_expressions(gt, target_expressions, exp_key, only_car):
    """
    Given "frame-level labels" and "video-level expression", get frame-level expressions and corresponding labels
    """
...
class RMOT_Dataset(Dataset):
    ...
    def __init__(self, mode, opt, only_car=False):
        ...
        self.opt = opt
        self.mode = mode
        self.only_car = only_car  # select category
            ...
            for obj_id, obj_label in labels[video].items():
                ...
                for value in obj_label.values():
                    if len(value['category']) > 0 \
                        and ((self.only_car and (value['category'][0] == 'car')) or (not self.only_car)):
                                num += 1
